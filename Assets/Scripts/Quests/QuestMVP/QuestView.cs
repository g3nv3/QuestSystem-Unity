...
public class QuestView : MonoBehaviour
{
    ...
    private Image highlightedImg;
    private bool hasSelected;
    private bool highlighted;
    private QuestData highlightedData;
    ...
    public void Load(List<QuestData> data)
    {
        foreach(QuestData item in data)
        {
            ...
            highlightedData = item;
            hasSelected = true;
            highlightedImg = obj.GetComponent<Image>();
            highlightedImg.color = _selectedColor;
            _selectedPanel.SetActive(true);

            QuestBus.GetInstance().OnSelect?.Invoke(highlightedData);
        }
    }
    ...
    private void Highlight(QuestData data, Image image)
    {
        ...
        highlighted = true;
        highlightedData.highlighted = true;

        _selectBtn.SetActive(highlighted);
        _btnSelectText.text = data.selected ? "Unselect" : "Select";
        highlightedImg.color = highlightedData.selected ? _selectedColor : _highlightedColor;
    }
    ...
    public void select()
    {
        ...
        hasSelected = !highlightedData.selected;
        highlighted = true;

        _presenter.select(highlightedData);
        _selectedPanel.SetActive(hasSelected);
        highlightedImg.color = hasSelected ? _selectedColor : _highlightedColor;
    ...
    {
        _presenter.UnselAll(highlightedData);
        foreach (var cell in cellsObj)
        {
            var img = cell.GetComponent<Image>();
    ...
    public void FinishQuest(QuestData data)
    {
        ...
        if(data.selected) _selectedPanel.SetActive(false);
